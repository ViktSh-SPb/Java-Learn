...
3. Relay помечает событие как отправленное только после успешной публикации в **обоих** системах. Если публикация в одной из них упала, relay ретраит (идемпотентность/deduplication важна).
Псевдо-логика relay:
```text

...
Stream API — это набор интерфейсов и методов, введённых в Java 8, который позволяет обрабатывать последовательности данных (например, коллекции) в функциональном стиле. Он обеспечивает декларативный подход к выполнению операций фильтрации, преобразования, агрегации и других над данными.
...
Лямбда-выражения позволяют задавать операции обработки элементов потока (например, фильтрацию, преобразование) компактно и читаемо. Они реализуют функциональные интерфейсы, необходимые для методов Stream API.
...
`collect()` — терминальный оператор, который собирает элементы потока в коллекцию или другую структуру данных. Обычно используется с классами из `Collectors`:
...
- Stateless операции: не зависят от предыдущих элементов или состояния потока (`filter()`, `map()`, `flatMap()`). Они могут быть выполнены независимо друг от друга.
...
`reduce()` объединяет элементы потока в один результат по заданной функции:

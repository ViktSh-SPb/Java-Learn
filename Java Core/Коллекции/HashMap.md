...
**HashMap** позволяет очень быстро <u>добавлять / искать / удалять</u> элементы по ключу. Не гарантирует порядок. Средняя сложность **О(1)**
Пример:
```java
...
```
>[!warning]
>Тип ключа должен быть immutable
### Null-значения
HashMap разрешает хранить null-значения. Null-ключ только один.
```java
map.put(null, "value");
map.put("x", null);
```
...
>[!warning]
**Критически важно!**
Если два объекта равны, то hashCode() должны возвращать одинаковые значнеия. Иначе HashMap ломается логически.
```java
a.equals(b) == true
...
| get      | O(1)    | O(n)   |
| remove   | O(1)    | O(n)   |
После treeification худший случай ближе к O(log n).
## Устройство HashMap
...
Этот процесс называется rehashing
### Load factor
По умолчанию - 0.75. Resize происходит когда: `size > capacity * loadFactor`
### Capacity
По умолчанию - 16. Можно задать:
```java
new HashMap(100);
```
Для быстрого вычисления индекса и равномерного распределения необходимо устанавливать capacity степень двойки (2, 4, 8, 16, 32, ...), т.к. для вычисления индекса вместо `hash % length`  используется `hash & (lengh - 1)`.
## Основные методы
...
| `compute(key, (k, v) -> ...)`              | Пересчитывает значение по функции                      |
| `computeIfAbsent(key, k -> ...)`           | Создает значение по функции только если его нет        |
## Альтернативы

| Коллекция         | Преимущества                 |
| ----------------- | ---------------------------- |
| LinkedHashMap     | Если нужен порядок вставки   |
| TreeMap           | Если нужна сортировка ключей |
